...
> Guoli Sun, Pascal Belleau, Astrid Deschênes and Alex Krasnitz (2020). CNprep: Pre-process DNA Copy Number (CN) Data for Detection of CN Events. R package version 2.1beta. https://github.com/belleau/CNprep
...
identified may not shared integer copy number. So, evaluating the value of 
...
The `r Rpackage("CNprep")` package has been implemented to solve this 
problematic. The `r Rpackage("CNprep")` package infers, through a multi-step 
process, the likely normal state of the cells and identify segments with 
significant copy number deviation from that state. To this end, 
`r Rpackage("CNprep")` uses both DNA copy number initial form (copy number 
as a noisy function of genomic position) and their 
...
to mask all inherited alterations. This function requires the copy number 
...
The *makeCNPmask()* function takes as an input: 
...
complexe dataset composed of 1203 breast cancer samples.
