...
genomic positions. First, interval regions where 
...
As an example, in Figure \@ref(fig:demoMask), the first section represent
...
thresholds selected for the analysis. The last section shows the result of
the #makeCNPmask* function for this dataset when the upper threshold is set to
0.45 and lower threshold to 0.25.

```{r demoMask, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Example"}
dataTest <- data.frame(chrom=rep("chr1", 10), start=c(12742927, 13071912,
    ...
    12742827), end=c(13039275, 13439275, 12939275, 13039175, 13551912, 
...
In Figure \@ref(fig:demoMask2), using a upper threshold of 0.55 on the same 
copy number events detected on 10 samples result in only one genomic region in
the mask. This region is the only one to respect both lower and upper 
thresholds. 

```{r demoMask2, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Example"}
dataTest <- data.frame(chrom=rep("chr1", 10), start=c(12742927, 13071912,
    12862827, 12708592, 13271912, 13071612, 12838592, 13091712, 12842827, 
    12742827), end=c(13039275, 13439275, 12939275, 13039175, 13551912, 
    13191012, 12930000, 13542827, 13456522, 13706522), 
...
## Adding upper threshold
add_lines_track(minThres, value=0.55, gp=gpar(col="red3", lty=3, lwd=2), 
                    track=current_track())
