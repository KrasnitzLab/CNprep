...
> Guoli Sun, Pascal Belleau, Astrid Deschênes and Alex Krasnitz (2020). CNprep: Pre-process DNA Copy Number (CN) Data for Detection of CN Events. R package version 2.1beta. https://github.com/KrasnitzLab/CNprep
...
computation. Parallelizing \code{CNpreprocessing } function can easily be 
accomplished by setting the \code{nJobs} parameter to a number higher than 
one. Beware that the parallelization is done per sample. So, the \code{nJobs} 
...
## The number of threads is set to 4 (nJobs parameters)
...
To ensure reproducible results, \code{set.seed()} function should be call 
before \code{CNpreprocessing}. Beware that the \code{nJobs} parameter must 
also be fixed. A change in the value of the \code{nJobs} parameter will 
