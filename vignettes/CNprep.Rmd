...
The *makeCNPmask* function takes as an input a table of copy number events with 
integer-valued boundary positions. Interval regions where 
the event count is above each of two specified thresholds, upper and lower, are 
identified and the function returns the interval regions with the count above 
the lower threshold that contain sub-interval regions with the count above 
the upper threshold.

```{r demoMask, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=6, fig.width=5}
dataTest <- data.frame(chrom=rep("chr1", 10), start=c(12742927, 13071912,
    12862827, 12708592, 13271912, 13071612, 12838592, 13091712, 12842827, 
    12942827), end=c(13239275, 13439275, 12939275, 13039175, 13551912, 
    13191012, 12930000, 13542827, 13456522, 13706522), value=seq(from=0.05, to = 0.95, by = .1))

library(GenomicRanges)
gr <- GRanges(
    seqnames = Rle(c("chr1"), c(10)),
    ranges = IRanges(start = dataTest$start, end = dataTest$end, names = head(letters, 10)),
    strand = Rle(strand(c("*")), c(10)))

covN <- GRanges(coverage(gr))
covN <- as.data.frame(covN)
covN <- covN[-1,]
covN2 <- covN
...
minThres <- data.frame(chrom=rep("chr1", 1), start=c(12708592, 13706522), end=c(12708592, 13706522))

library(gtrellis)
gtrellis_layout(data=dataTest, xpadding = c(0.015, 0.015), n_track = 2, 
                track_ylim = c(0, 1, 0, 0.75), add_name_track = TRUE,
                track_height = unit.c(unit(1, "null"), unit(2, "null")),
                track_axis = c(FALSE, TRUE),  
                track_ylab = c("Copy Number Event", "Event Frequency"))

add_segments_track(dataTest, value=dataTest$value, gp=gpar(col="darkorange3", lwd=4))
add_lines_track(covNT, value=covNT$freq, gp=gpar(lwd=3, col="darkblue"))


add_lines_track(minThres, value=0.25, gp=gpar(col="red3", lty=4, lwd=2), track = current_track())

add_lines_track(minThres, value=0.45, gp=gpar(col="red3", lty=3, lwd=2), track = current_track())


```


```{r makeMask, echo=TRUE}
...
```{r treillisMask, echo=TRUE, message=FALSE, warning=FALSE}
ampCNP2 <- ampCNP[ampCNP$chrom ==1, c("chrom", "chrom.start", "chrom.end")]
ampCNP2$chrom <- paste0("chr", ampCNP2$chrom)
ampCNP2$value <- rep(1, nrow(ampCNP2))

library(gtrellis)
gtrellis_layout(category = c("chr1"))
add_segments_track(ampCNP2, value = 0.5)
